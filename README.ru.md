...
### Базы данных (v1.3)
- **PostgreSQL, MySQL/MariaDB и Redis** каналом внутри уже поднятой SSH-сессии - проброс
  порта поднимать не нужно
...
- Клиент MySQL свой, на основе `mysql_common`
...
- **`Serein_1.3.1_x64-portable.exe`** - один файл Windows, без установки. Положи и запусти. Настройки всё равно в `%APPDATA%\serein`. В 1.3.1 в этом файле **нет RDP**: удалённому рабочему столу нужен помощник `serein-rdp.exe` рядом, а однофайловая сборка его не несёт. Со следующего выпуска переносная сборка - ZIP с обоими файлами.
- **`Serein_1.3.1_amd64.deb`** - пакет Debian/Ubuntu/**Astra** (`/usr/bin/serein`).
- **`Serein-1.3.1-1.x86_64.rpm`** - пакет **Fedora**. Ставится и запускается на чистой Fedora; на РедОС и Альт пока не проверялся.
...
- В однофайловом portable `.exe` выпуска 1.3.1 нет RDP: помощник - отдельная программа, и
  один файл его не несёт. Бери установщик или portable ZIP со следующего выпуска.
