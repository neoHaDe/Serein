...
Бесплатно, открытый код, Apache 2.0. Windows x64 и Linux x64, **v1.3.1**.
...
| --- | :---: | :---: |
| Размер установщика | **≈ 4,4 МБ** | ≈ 85 МБ |
...
Память меряна на установленной 1.3.0 без единой открытой сессии: 100 МБ приватной на всё
дерево из семи процессов. Если сложить Working Set в диспетчере задач, выйдет около 460 МБ,
но это та же память, посчитанная по разу на каждый процесс WebView2, которые её делят.
Числа Electron - публичные замеры, приведённые к той же метрике. Размер - установщик 1.3.0.
...
- **VNC и RDP каналом внутри SSH-сессии**, без открытого наружу порта. И правильно: свой
  протокол VNC защищает паролем до восьми символов на DES, да и RDP в сети делать нечего
...
- **RDP спрашивает вход один раз (v1.3.1).** Имя и пароль уходят серверу с признаком
  готового входа, и он не показывает поверх картинки своё окно. Настройки: разрешение,
  глубина цвета, экономия трафика. Разрешение следует за окном - растянул, и сервер
  перерисовал рабочий стол в новом размере, без переподключения
- **Протокол разбирает отдельная программа** рядом с приложением: IronRDP прибивает
  шестнадцать криптографических крейтов к release candidate'ам, с которыми SSH-ядро не сходится
...
- **Шестерёнка ставит серверную часть, если её нет:** `x11vnc` или `tigervnc-server` для
  ...
  строку команды: строка целиком видна в списке процессов сервера всем, кто там есть
...
1. Поставь установщик или скачай portable `Serein_1.3.1_x64-portable.exe` из [Releases](../../releases/latest).
...
- **`Serein_1.3.1_x64-setup.exe`** - установщик Windows (меню Пуск, удаление).
- **`Serein_1.3.1_x64-portable.exe`** - один файл Windows, без установки. Положи и запусти. Настройки всё равно в `%APPDATA%\serein`.
- **`Serein_1.3.1_amd64.deb`** - пакет Debian/Ubuntu/**Astra** (`/usr/bin/serein`).
- **`Serein-1.3.1-1.x86_64.rpm`** - пакет **Fedora** и родственных (РедОС, Альт).
- **`Serein_1.3.1_amd64.AppImage`** - портативный Linux-бинарь, один файл на оба семейства.
...
Что нового - [release notes](docs/RELEASE_NOTES_v1.3.1.md) и описание выпуска на GitHub.
...
**VNC**, **базы данных** поверх своих же каналов и **Windows Server**, а 1.3.1 добавил
**RDP**. Осталось: остальные три базы (SQLite, MongoDB, SQL Server), macOS и реестр
отечественного ПО. Подробнее - [release notes 1.3.1](docs/RELEASE_NOTES_v1.3.1.md).
