...
export interface QueryResult {
  ...
  truncated?: boolean
  /**
   * Все наборы результатов запроса.
   *
   * Несколько операторов подряд и хранимые процедуры отвечают не одной выборкой. Раньше
   * показывалась первая, остальные выбрасывались - человек видел пустую таблицу там, где
   * данные пришли вторым набором.
   */
  sets?: QuerySet[]
  /** Какой набор показан в таблице по умолчанию: первый со строками. */
  shown?: number
}
