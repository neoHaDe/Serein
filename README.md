...
Free, open source, Apache 2.0. Windows x64 and Linux x64, **v1.5.2**.
...
- **`Serein_1.5.2_x64-setup.exe`** - Windows installer (Start menu, uninstall).
- **`Serein_1.5.2_x64-portable.zip`** - portable Windows build, no installer: `Serein.exe` and the RDP helper `serein-rdp.exe`. Unpack into a folder and run. Settings still live in `%APPDATA%\serein`.
- **`Serein_1.5.2_amd64.deb`** - Debian/Ubuntu/**Astra** package (`/usr/bin/serein`).
- **`Serein-1.5.2-1.x86_64.rpm`** - **Fedora** package. Installs and runs on a clean Fedora; on RedOS and Alt it has not been checked yet.
- **`Serein_1.5.2_amd64.AppImage`** - portable Linux binary, one file for both families.
- **`Serein-user-guide-ru.pdf`** - the user guide in PDF, in Russian like the interface; the same
  guide with screenshots is in [docs/guide](docs/guide/README.md).
...
Release notes: [RELEASE_NOTES_v1.5.2.md](docs/RELEASE_NOTES_v1.5.2.md).
...
action log record what actually happened; 1.5.2 brought back confirmations before irreversible
actions and added a user guide. Still open: macOS, the Russian software registry, a shared
catalog, roles and SSO. See [release notes 1.5.2](docs/RELEASE_NOTES_v1.5.2.md).
