...
fn busy_text(dir: &Path) -> String {
    format!(
        "Serein уже открыт с этим профилем:\n{}\n\nВторой экземпляр не запускается: два процесса переписывали бы серверы и пароли друг другу, и изменения одного молча пропадали бы. Переключитесь на открытое окно - оно может быть свёрнуто или спрятано в трей: значок Serein рядом с часами.",
        dir.display()
    )
}
