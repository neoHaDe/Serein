...
///
/// Нужна, когда панель появляется на пустом месте и не помнит ничего: откреплённое
/// окно - это отдельный веб-контекст со своей памятью, а соединение живёт в приложении
/// и переезд окна переживает. Без этого вопроса отделение панели выглядело бы как обрыв
