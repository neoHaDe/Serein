...
//!
//! ⚠ Живого теста здесь нет намеренно. Любое обращение к SSH-стеку из тестового бинаря
//! этого крейта ломает его ещё на загрузке (`STATUS_ENTRYPOINT_NOT_FOUND`) — падают **все**
//! тесты, а не только новый. Проверено: дело не в `#[tokio::test]`, ручной рантайм даёт
//! то же самое. Поэтому здесь только чистая логика, а связка целиком проверяется руками.
